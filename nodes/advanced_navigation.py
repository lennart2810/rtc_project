...
class SetNavigationPoints(object):
    def __init__(self, filename, controller_layout, map_file):
        ...
        #rospy.Subscriber('amcl_pose', PoseWithCovarianceStamped, self.cb_get_amcl_pose)
        rospy.Subscriber('tf', TFMessage, self.cb_get_tf_pose)
        self.pose = Pose()
    ...
    def cb_scan(self, scan):
        ...
        mean_front_ranges = [scan.ranges[:5], scan.ranges[-5:]]
        mean_front_ranges = np.asanyarray(mean_front_ranges)
        mean_front_ranges = mean_front_ranges.reshape(10,).T

        for i, scan_i in enumerate(mean_front_ranges):
            if scan_i == float('Inf'):
                mean_front_ranges[i] = 3.5
            elif isnan(mean_front_ranges[i]):
                mean_front_ranges[i] = 0

        # self.distance = scan_filter[0]  # nur frontal scan
        self.distance = np.mean(mean_front_ranges) 
